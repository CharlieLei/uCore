...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    // dup_files要求proc的file_struct已经初始化，因此不能用dup_files
...
load_icode(int fd, int argc, char **kargv) {
    ...
    struct elfhdr elf_phy, *elf = &elf_phy;
    ...
    uint32_t vm_flags, perm, phid = 0;
    struct proghdr ph_phy, *ph = &ph_phy;
    for (; phid < elf->e_phnum; phid ++) {
        off_t phoff = elf->e_phoff + phid * sizeof(struct proghdr);
        ...
        while (start < end) {
            ...
            if ((ret = load_icode_read(fd, page2kva(page) + off, size, part_offset)) != 0) {
                goto bad_cleanup_mmap;
            }
            start += size, part_offset += size;
        }
